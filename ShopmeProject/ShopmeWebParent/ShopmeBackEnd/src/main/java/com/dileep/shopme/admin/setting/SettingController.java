...
public class SettingController {
	...
	@Autowired
	private ICurrrencyRepository currrencyRepository;
	
	@GetMapping("/settings")
	public String listAll(Model model) {
		
		List<Setting> listSettings = settingService.listAllSettings();
		List<Currency> listCurrencies = currrencyRepository.findAllByOrderByNameAsc();
	...
	public String saveGeneralSettings(@RequestParam("fileImage") MultipartFile multipartFile ,HttpServletRequest request,
		...
		System.out.println("settingBag" + settingBag.get("SITE_LOGO"));
		updateSettingValuesFromForm(request, settingBag.list());
		
		ra.addFlashAttribute("message","General setings have been saved.");
	...
	private void saveCurrencySymbol(HttpServletRequest request,GeneralSettingBag settingBag) {
		...
		Optional<Currency> findByIdResult = currrencyRepository.findById(currencyId);
